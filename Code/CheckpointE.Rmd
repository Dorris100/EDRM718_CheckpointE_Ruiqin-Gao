...
ggplot(aes(x=hosp.beds, y=doctors))+
  ...
  geom_smooth()+
  labs(y="Number of physicians", x="Number of beds in hospital")
```
###5
```{r}
...
health %>%
  ...
  labs(y="Number of physicians", x="Number of beds(>1000) in hospital")
```
###7
```{r echo=TRUE}
...
health4<-health3 %>%
  filter(hosp.beds <1000 ) %>%
  ggplot(aes(x=hosp.beds, y=doctors, color=DensityLevel))+
  geom_point(color= "red", size=3, shape=1)+
...
health4<-health3 %>%
  ...
  geom_point(color= "red", size=3, shape=1)+
  geom_smooth(method="lm",se=FALSE)+
  labs(y="Number of doctors",x="Number of hospital beds",
